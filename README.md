...
- Change the stink level of Roquefort to 3
  ...
  ```ruby
  roque = Cheese.where({name: "Roquefort"})
  roque.stink_level = 3
  roque.save
  ```
...
- What does the command `rails c` do?
  ```
  opens a console with rails environment
  ```
...
- What does a migration file do?
  ```
  modifies the database, adds tables with specified columns
  ```
...
- What is the command to start a new rails API called "reasons_why_ghadeer_rules"?
  ```
  rails new reasons_why_ghadeer_rules -G --api
  ```
- What is an API?
  ```
  A website that serves information, with no views
  ```
